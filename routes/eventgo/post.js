...
function() {
  ...
  var model = require('../../models/eventgo-elasticsearch/post');
  var queryLogModel = require('../../models/eventgo-elasticsearch/query-log');


  module.exports.getSearchExtractedEvent = function(req, res, next) {
    async.waterfall([
      // Check parameters.
      function(cb) {
        var ret = parseGeExtractedEventParams(req.query);
        cb(null, ret.params);
      },
      function(params, cb) {
        // elastic logjh
      ...
      function(params, cb) {
        ...
        if ((opts.limit > 0) && (opts.limit <= 100)) {
          return model.getExtractedEvent(opts, function(err, events) {
            var error = {
  ...
  function parseGeExtractedEventParams(params) {
    ...
    if ('sort' in params) {
      if (!/^(start_time|end_time|num_mood|num_likes|num_angry|num_ha|num_wow|_score)$/.test(params.sort)) {
        return { err: '`sort` must be `start_time`, `end_time`, `num_mood`, `num_likes`, `num_angry`, `num_ha`, `num_wow` or `_score`.' };
      }
